...
namespace Ghj
{
    public partial class Passing_the_test : Form
    {
        // подключение к бд
        ...
        static public int Score = 0, Oc = 0;
        static public double Correct_answers = 0;
        static public string Time;

        string[] answers = new string[5];
        public Passing_the_test()
        {
            InitializeComponent();
        }

        private void Passing_the_test_Load(object sender, EventArgs e)
        {
            Con.OpenConnectic();
            ...
            // получние таблицы в вопросами из бд
            string query = "SELECT COUNT(*) FROM Q.Questions";
            int count = Convert.ToInt32(Con.Select(query)) + 1;

            // получение рандомных id вопросов и присвоение этих id переменным
            for (int i = 0; i <= 5; i++)
            {
            ...
            // полкчение не правильных ответов
            for(int i = 1; i <=3; i++)
            {
            ...
            // вывод ответов в случайном порядке
        ...
        private void finish_the_test_Click(object sender, EventArgs e)
        {
            // проверка отмеченных ответов на правильность 
            ...
            else if(Correct_answers / 5 >= 0.8)
            {
                Oc = 4;
            }
            else if(Correct_answers / 5 >= 0.6 && Correct_answers / 5 < 0.8)
            {
                Oc = 3;
            }
            else if (Correct_answers / 5 >= 0.4 && Correct_answers / 5 < 0.6)
            {
                Oc = 2;
            }
            else if(Correct_answers / 5 < 0.4)
            {
                Oc = 1;
            }
            ...
            label2.Text = $"Баллов за тест {Score}, Правильных ответов {Correct_answers}/5, Время прохождения теста {Time}, Оценка {Oc}";

            // внесение результатов теста в бд
            query = $"INSERT INTO `q`.`results` (`Логин`, `Баллы`, `Время_выполнения`, `Оценка`) VALUES ('{Login.login}', {Score}, '{Time}', {Oc})";
            Con.Select(query);
            Correct_answers = 0;
            Score = 0;
            Oc = 0;
